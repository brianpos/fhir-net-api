...
{
    public class PocoElementNode : ITypedElement, IAnnotated, IExceptionSource, IShortPathGenerator, IFhirValueProvider, IResourceTypeSupplier
    {
        ...
        private PocoElementNode(Base instance, PocoElementNode parent, IElementDefinitionSummary definition, string location, string shortPath, string commonPath, int arrayIndex)
        {
            Current = instance;
            _mySD = (PocoComplexTypeSerializationInfo)PocoStructureDefinitionSummaryProvider.Provide(Current.GetType());
        ...
        public IEnumerable<ITypedElement> Children(string name)
        {
            ...
            foreach (var child in children)
            {
                if (name == null || child.ElementName == name)
                {
                    ...
                    yield return new PocoElementNode(child.Value, this, childElementDef, location, shortPath, commonPath, arrayIndex);
                }
